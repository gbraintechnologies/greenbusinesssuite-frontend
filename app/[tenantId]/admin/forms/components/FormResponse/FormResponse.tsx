...
const renderFormResponse = (formField: any) => {
  switch (formField?.fieldDataType) {
    ...
    case "long-text":
      return (
        <textarea className=" big-fit" readOnly disabled>
          {formField?.response}
        </textarea>
      );
...
const FormResponse = forwardRef(function FormResponse(
  ...
  return (
    <div className="w-full " ref={ref}>
      {formSections
        ?.filter((section: any) => section?.isDeleted !== true)
        ?.filter(
          (section: any) => section.formFields && !section.formDataFields
        )
        ?.map((section: any, index: number) => (
          <div
            className="bg-white rounded-lg mb-4 w-full px-4 py-5"
            key={index}
            id={index > 0 ? "newsection" : "firstsection"}
          >
            ...
            <div className="form-container">
              {section?.formFields?.map((formField: any, index: number) => {
                ...
                const itemClass = isHorizontalAlign
                  ? "half-width"
                  : "full-width";
