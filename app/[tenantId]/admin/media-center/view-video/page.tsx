...
function ViewVideo({ params }: any) {
  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    queryFn: services.getMediaTypeByID(Number(videoId)),
    enabled: !!videoId,
  });
            ...
            <div className="w-full text-primary-dark flex pt-4 justify-between">
              <div>
                <div className="flex items-center gap-3 mb-10">
                  ...
                  <h3 className="font-semibold text-xl">View Video</h3>
                </div>
              </div>
              <div className="flex gap-3">
                <Link href={`/${tenantId}/admin/media-center/edit-video?id=${data?.id}`}>
                  <button
                    type="button"
